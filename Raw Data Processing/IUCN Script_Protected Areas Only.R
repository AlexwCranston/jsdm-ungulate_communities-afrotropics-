...
## Read in rhino shapefiles from Rookmaaker and Antoine


C.simum <- readOGR("Rhino Range Map Processing/Outputs", "White_Rhino_Range")
C.cottoni <- readOGR("Rhino Range Map Processing/Outputs", "Nile_Rhino_Range")
D.bicornis <- readOGR("Rhino Range Map Processing/Outputs", "Black_Rhino_Range")

## Add data to IUCN maps from Rookmaaker/Antoine maps

C.simum@data <- combined.polygons[25,]@data 
C.cottoni@data <- combined.polygons[25,]@data 
D.bicornis@data <- combined.polygons[32,]@data

# Make final polygons set

combined.polygons <- combined.polygons[-c(25,32),] # Drop IUCN rhino maps
combined.polygons <- rbind(combined.polygons, C.simum, C.cottoni, D.bicornis) # replace with Rookmaaker/Antoine maps


combined.polygons <- st_as_sf(combined.polygons) # convert to sf object for quicker processing in letsR
...
PAM_All <- lets.pamcrop(PAM, protected.areas, remove.sp = TRUE)

presence.absence.matrix<-PAM_All$P
...
PAM_Open <- lets.pamcrop(PAM, protected.areas_OpenHabitats, remove.sp = TRUE)

presence.absence.matrix<-PAM_Open$P
presence.absence <- data.frame(presence.absence.matrix)

presence.absence <- presence.absence %>% filter(Latitude.y.<= 15) # Exclude all points above 15 degrees North, i.e. include only points from sub Saharan Africa

# Combine two White Rhino subspecies into one column
...
PAM_Tropical <- lets.pamcrop(PAM, protected.areas_TropicalForest, remove.sp = TRUE)

presence.absence.matrix<-PAM_Tropical$P
presence.absence <- data.frame(presence.absence.matrix)
