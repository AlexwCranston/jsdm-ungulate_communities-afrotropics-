...
load("models/models_thin_100_samples_250_chains_4_FINALRUN.Rdata") # Load models
m<-models[[2]] # Take Open Habitats model

postEta = getPostEstimate(m, parName ="Eta") # Extract eta parameters
xy<-models[[2]]$rL$ID$s # Extract coordinates
...
openHabitats_shapefile <- readOGR("Raw Data Processing", "Africa_minusTropicalForests") # upload shapefile
proj4string(openHabitats_shapefile) <- CRS("+proj=longlat +datum=WGS84") # add crs
openHabitats_shapefile<-spTransform(openHabitats_shapefile, CRS("+proj=merc +datum=WGS84")) # Transform to merc
openHabitats_shapefile.shp = openHabitats_shapefile@polygons # Convert to spatial polygons from spdf
...
grid.1 = grid.1[!is.na(over(grid.1, openHabitats_shapefile.shp)),] # Crop grid to biome area and check grid is correct with plot(grid.1)
